...
function processCommits(data) {
  return d3
    ...
    .map(([commit, lines]) => {
      ...
      let ret = {
        id: commit,
        url: 'https://github.com/alexisjacques/portfolio/commit/' + commit,
        author,
        date,
        time,
        timezone,
...
// Render commit information and statistics
function renderCommitInfo(data, commits) {
  ...
  dl.append('dd').text(
    `${longestLineEntry.length} characters in ${longestLineEntry.file}:${longestLineEntry.line}`,
  );
...
// Render scatterplot of commits by time of day
function renderScatterPlot(data, commitsData) {
  // Assign to global variable for brush functions
  commits = commitsData;
  ...
  // Create scales - assign to global variables for brush functions
  xScale = d3
    .scaleTime()
  ...
  // Create radius scale for dot sizes based on lines edited
  ...
  // Raise dots and everything after overlay to fix tooltip interaction
